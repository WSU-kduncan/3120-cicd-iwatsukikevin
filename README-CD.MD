...
- I could not remember how I installed Docker on my instance before, so I used the following steps:
...
- How to start the webhook:
  ...
  - Next, go to a linux terminal that is outside of the AWS instance and use the command `curl http://yourinstanceIP:9000/hooks/webhook-file-id`. If successfull,  information should appear on the instance terminal telling the user the webhook has been found and `restart.sh` has been activated.
...
- DockerHub:
  ...
  - Click on `Webhooks` and fill out the text lines with the webhook name and URL.
  - Click `Create` when finished.
  - To view history, go to the current webhooks block and press the three-dot column at the upper-right corner of your webhook URL. Select "view history".
  - You should see the history of the past webhook activiations, but it may take a while to show anything.
